...
Tinder-style Movie-Matching-App: zwei User adden sich via Share-Code als Partner, swipen gemeinsam auf Filme und erhalten eine Match-Benachrichtigung (inkl. Streaming-Verfügbarkeit via JustWatch) wenn beide denselben Film liken.
...
└── tests/
    ...
    └── integration/      # auth (Register-share_code, Cascade auf Partnerships),
                          ...
                          # auth-google (503 ohne Config, 400 missing idToken, 401 verify-fail,
                          # 401 null sub, 201 Neu-User, 200 existing user, email linking,
                          # Name-Fallback, kein Email),
...
└── google-signin-setup.md   # Google Cloud Console → OAuth Consent → iOS Xcode → Env Vars → App Store Freigabe
```
...
GOOGLE_CLIENT_ID_WEB=     # Web Client ID (optional — für zukünftigen Web/Android-Support)
...
| `POST`   | `/api/auth/google`                                   | Google Sign-In: ID-Token verify via google-auth-library, 3-step find-or-create (google_id → email → create)         |
...
| Status        | Thema                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **in Arbeit** | Partnerships-Refactor: Rooms → persistente Partnerschaften, Gast-Zugang weg, Share-Codes mit Double-Opt-In, Apple Sign-In, Google Sign-In. Plan + Phasen: `docs/partnerships-refactor-plan.md` (Parent-Repo). Branch: `refactor/partnerships`. **Phasen 1–10 fertig** (Phase 10 am 2026-05-03): Backend (Schema/Services/Routes/Socket/Tests, 136 Tests grün), iOS-Stack (Models/Services/ViewModels/Views) und Deep-Link/Push-Routing sind umgebaut. Phase 9 (Apple Sign-In) + Phase 10 (Google Sign-In) abgeschlossen: Backend-Route `/api/auth/google` (google-auth-library, 3-step find-or-create), iOS `GoogleSignInHelper`, `APIService.googleSignIn`, `AuthViewModel.signInWithGoogle`. iOS: SPM-Package `GoogleSignIn-iOS` muss noch in Xcode hinzugefügt werden + reversed-Client-ID URL-Scheme (`com.googleusercontent.apps.600845465744-4cjhu5pv0fnslqfbmtjf8r4tcm54buki`). iOS-Build zuletzt grün am 2026-04-28 via `xcodebuild -quiet ... CODE_SIGNING_ALLOWED=NO build` (außerhalb Sandbox wegen CoreSimulator-Zugriff). |
...
| **post-MVP**  | Partnership-Namen editieren in UI (keine Route — Partnerschaft wird derzeit über Partner-Username identifiziert)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
